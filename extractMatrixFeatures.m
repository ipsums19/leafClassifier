function [setFeatures] = extractMatrixFeatures(imgData)
    ...
    nFeatures = [1024 512 1 1 1 1 1 1 1];
    ...
    for i = 1:nFiles
        ...
        Itmp = imresize(IStem, [h w]);
        setFeatures(i, (sum(nFeatures(1:2))+1):sum(nFeatures(1:3))) = sum(sum(Itmp));

        %'Eccentricity', 'Solidity', 'Extent'   
        CC = bwconncomp(BW);
        P = regionprops(CC, 'Extent', 'Eccentricity', 'Solidity');
        ...
        CC = bwconncomp(ILeaf);
        P = regionprops(CC, 'Extent', 'Eccentricity', 'Solidity');
        setFeatures(i, sum(nFeatures(1:7))) = P.Extent;
        setFeatures(i, sum(nFeatures(1:8))) = P.Eccentricity;
        setFeatures(i, sum(nFeatures(1:9))) = P.Solidity;  
